...
#         self.z = pp.Parameter(pp.randn_so3((2, 3), requires_grad=True, device=device))  # Lie Tensor
...
class ADMMModelSE3(nn.Module):
    def __init__(self, *dim) -> None:
        ...
        random_se3 = pp.Parameter(pp.randn_SE3(*dim).to(device))
        print(random_se3)
...
if __name__ == "__main__":
    ...
    admm_model = ADMMModelSE3(1).to(device)
    ...
    inner_optimizer_x = torch.optim.SGD(admm_model.parameters(), lr=1e-4)
    inner_schd_x = torch.optim.lr_scheduler.StepLR(optimizer=inner_optimizer_x, step_size=10, gamma=0.5)
    inner_optimizer_z = torch.optim.SGD(admm_model.parameters(), lr=1e-4)
    inner_schd_z = torch.optim.lr_scheduler.StepLR(optimizer=inner_optimizer_z, step_size=10, gamma=0.5)
    optimizer = ADMMOptim(admm_model.parameters(), inner_optimizer_x, inner_optimizer_z)
