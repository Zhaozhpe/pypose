...
# class ADMMOptim(_Optimizer):
#     def __init__(self, model, inner_optimizer_x, inner_optimizer_z, rho=0.5, tolerance=3e-4,min=1e-6, max=1e32):
#         defaults = {**{'min':min, 'max':max}}
#         super().__init__(model.parameters(), defaults=defaults)
#         self.terminate = False
#         self.model = model
...
#         for i in range(self.inner_iter):
#             self.inner_optimizer_x.zero_grad()
#             loss_x = self.model.obj(inputs) + self.model.u.T *self.model.cnst(inputs)+ self.rho/2 *torch.norm(self.model.cnst(inputs),p=2) ** 2
#             scalar_loss_x = loss_x.sum()
#             scalar_loss_x.backward()
#             self.inner_optimizer_x.step()
...
#         for j in range(self.inner_iter):
#             self.inner_optimizer_z.zero_grad()
#             loss_z = self.model.obj(inputs) +self.model.u.T*self.model.cnst(inputs) + self.rho/2 * torch.norm(self.model.cnst(inputs),p=2) ** 2
#             scalar_loss_z = loss_z.sum()
#             scalar_loss_z.backward()
#             self.inner_optimizer_z.step()
...
class ADMMOptim(_Optimizer):
    def __init__(self, model, inner_optimizer_x, inner_optimizer_z, rho=0.5, tolerance=3e-4,min=1e-6, max=1e32):
        ...
        self.inner_iter = 300
        self.tolerance = tolerance
        self.w = torch.zeros_like(self.model.u)

    def step(self, inputs):
        z_old = self.model.z.clone()

        with torch.no_grad():
            fx, gx, obj, cnst = self.model(inputs)
            self.last_object_value = obj.sum()
            self.last_violation = torch.norm(cnst)

        for i in range(self.inner_iter):
            self.inner_optimizer_x.zero_grad()
            loss_x = self.model.fx + self.w.T * self.model.cnst(inputs) + 0.5 * self.rho * torch.norm(self.model.cnst(inputs), p=2) ** 2
            scalar_loss_x = loss_x.sum()
            scalar_loss_x.backward()
            self.inner_optimizer_x.step()
        ...
        for j in range(self.inner_iter):
            self.inner_optimizer_z.zero_grad()
            loss_z = self.model.gx + self.w.T * self.model.cnst(inputs) + 0.5 * self.rho * torch.norm(self.model.cnst(inputs), p=2) ** 2
            scalar_loss_z = loss_z.sum()
            scalar_loss_z.backward()
            self.inner_optimizer_z.step()
        ...
        with torch.no_grad():
            # Update w instead of u
            self.w += self.model.cnst(inputs)

            fx, gx, object_value, violation = self.model(inputs)
            self.object_value = object_value.sum()
            self.violation_norm = torch.norm(violation)
            dual_residual = torch.norm(self.model.z - z_old)

            z_old = self.model.z.clone()
            self.best_violation = torch.norm(violation)

        return self.model.obj(inputs), self.w, self.violation_norm, dual_residual
